...
public class HPerfMain
{
	...
	//                     STATIC METHODS
	//----------------------------------------------------------
	public static void init( URL[] modules, String[] args ) throws Exception
	{
		...
		if( configuration.isJvmFederation() )
			executeJvmFederation( configuration, modules );
		else		
		...
		for( String name : allFederates )
		{
			...
			Runnable runner = new Runnable()
			{
				public void run()
				{
					try
					{
						new TestRunner(local).execute( modules );
					}
